...
# ── Đợi cả hai healthy ────────────────────────────────────────
info "Đợi services ready (tối đa 60s mỗi service)..."
sleep 4
 
for entry in "chat-bot:${CHAT_PORT}" "stt-bot:${STT_PORT}"; do
    name="${entry%%:*}"; port="${entry##*:}"
    HEALTHY=false
    for i in $(seq 1 30); do   # 30 × 2s = 60s
    ...
    if [ "$HEALTHY" = false ]; then
        warn "$name chưa healthy sau 60s — xem $LOG_DIR/${name}.log"
    fi
done
...
# ── Register lần đầu, sau đó in summary ──────────────────────
register_service "chat" "$CHAT_URL"
register_service "stt"  "$STT_URL"
...
# ── Block ở đây — heartbeat loop vô tận ──────────────────────
# QUAN TRỌNG: phải block, không thì script exit → trap cleanup
# kill hết chat-bot/stt-bot ngay lập tức.
heartbeat_loop